...
CREATE TABLE `agents` (
  ...
  `dtReported` TIMESTAMP NULL DEFAULT NULL COMMENT 'Last Date & time it reported back',
  PRIMARY KEY (`id`)
...
CREATE TABLE `monitoring_session` (
  ...
  `sessionend` TIMESTAMP NULL DEFAULT NULL COMMENT 'Session End Data and Time',
  `appversion` VARCHAR(45) DEFAULT NULL,
  `appcomany` VARCHAR(255) DEFAULT NULL,
  `applastaccess` VARCHAR(45) DEFAULT NULL,
  `applastwrite` VARCHAR(45) DEFAULT NULL,
  `createddatetime` VARCHAR(45) DEFAULT NULL,
  PRIMARY KEY (`id`)
...
/* Procedure structure for procedure `sp_updateSessionEnded` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_updateSessionEnded` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`bsap_admin`@`%` PROCEDURE `sp_updateSessionEnded`()
BEGIN
	DECLARE done INT DEFAULT 0;
...
/*!50001 CREATE TABLE  `web_view_monitoring_session`(
 ...
 `appcomany` varchar(255) ,
 `applastaccess` varchar(45) ,
 `applastwrite` varchar(45) ,
 `createddatetime` varchar(45) ,
 `computername` varchar(45) ,
 `maxmem` varchar(45) ,
 `cpuspeed` varchar(45) ,
 `cpuname` varchar(255) ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`bsap_admin`@`%` SQL SECURITY DEFINER VIEW `web_view_app_project_name` AS select `apn`.`id` AS `id`,`apn`.`name` AS `name`,`apn`.`appdesc` AS `appdesc`,concat((case `apn`.`enabled` when 1 then 'YES' when 0 then 'NO' end)) AS `enabled`,cast(concat('<a href="Project_list_all_sessions.aspx?ID=',`apn`.`id`,'">',`apn`.`name`,'</a>') as char(255) charset utf8) AS `web_name`,cast(concat('<a href="Project_list_all_sessions_by_agent.aspx?ID=',`apn`.`id`,'">',convert(concat(_latin1'<img src="images/computer.png"') using utf8),'</a>') as char(255) charset utf8) AS `byagent`,(select count(0) AS `count(0)` from `monitoring_session` where (`monitoring_session`.`APNID` = `apn`.`id`)) AS `TotalSessions`,cast(concat('<a href="Edit_Application.aspx?ID=',`apn`.`id`,'">Edit</a>') as char(255) charset utf8) AS `EDITME` from `app_project_name` `apn` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`bsap_admin`@`%` SQL SECURITY DEFINER VIEW `web_view_monitoring_session` AS select `ms`.`id` AS `id`,`ms`.`APNID` AS `apnid`,`apn`.`name` AS `ProjectName`,cast(concat('<a href="Project_list_all_sessions?ID=',`ms`.`id`,'">',`apn`.`name`,'</a>') as char(255) charset utf8) AS `web_ProjectName`,`ms`.`AID` AS `aid`,`ms`.`sessiondt` AS `sessiondt`,`ms`.`sessionend` AS `sessionend`,`ms`.`appversion` AS `appversion`,`ms`.`appcomany` AS `appcomany`,`ms`.`applastaccess` AS `applastaccess`,`ms`.`applastwrite` AS `applastwrite`,`ms`.`createddatetime` AS `createddatetime`,`a`.`computername` AS `computername`,`a`.`maxmem` AS `maxmem`,`a`.`cpuspeed` AS `cpuspeed`,`a`.`cpuname` AS `cpuname`,`a`.`operatingsystem` AS `operatingsystem`,cast(concat('<a href="Session_Details.aspx?SessionID=',`ms`.`id`,'&APNID=',`ms`.`APNID`,'">',`ms`.`sessiondt`,'</a>') as char(255) charset utf8) AS `web_sessiondt`,cast(concat('<a href="Session_Details_by_agent.aspx?SessionID=',`ms`.`id`,'&APNID=',`ms`.`APNID`,'&AID=',`ms`.`AID`,'">',`a`.`computername`,'</a>') as char(255) charset utf8) AS `web_computername`,(case (select count(0) AS `count(0)` from `logs_main` where (`logs_main`.`sessionid` = `ms`.`id`)) when 0 then 'NO' else 'YES' end) AS `HasLogs`,cast(concat('<a href="Project_list_all_sessions?DID=',`ms`.`id`,'&ID=',`ms`.`APNID`,'">delete</a>') as char(255) charset utf8) AS `web_deletesession` from ((`monitoring_session` `ms` join `agents` `a` on((`a`.`id` = `ms`.`AID`))) join `app_project_name` `apn` on((`apn`.`id` = `ms`.`APNID`))) */;
